...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(varyable => {
    varyable = varyable.split('');
    varyable[0] = varyable[0].toUpperCase();
    return varyable.join('');
  });
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(varyable => parseInt(varyable.mass) > 77).map(varyable => varyable.name).join(" - ");
};
