...
const raisedToTheThird = (arr) => {
  // Solution code here...
  let myArray = [];
  arr.forEach((num) => [myArray.push(Math.pow(num, 3))]);
  return myArray;
};
...
const addOne = (arr) => {
  ...
  let Value = arr.map((ferstNum) => ferstNum + 1);
  return Value;
};
...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    arrayele.push(Math.pow(2, arr[i]));
  }
  return arrayele;
};
...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  let Array1 = [];
  arr.forEach((number) => [Array1.push(Math.pow(2, number))]);
...
const mapTwoToThe = (arr) => {
  ...
  let array2 = arr.map((number) => Math.pow(2, number));
